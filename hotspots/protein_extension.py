...
class Protein(protein.Protein):
    def __init__(self, identifier, _molecule=None, _protein_structure=None, _cell=None):
        super().__init__(identifier, _molecule=_molecule,
                         _protein_structure=_protein_structure, _cell=_cell)
    ...
    class BindingSiteFromGrid(protein.Protein.BindingSiteFromListOfAtoms):
        def __init__(self, prot, grid, within=4, background_val=0):
            list_of_residues = self._detect_from_grid(
                prot, grid, within, background_val)
            self.list_of_atoms = tuple(
                a for r in list_of_residues for a in r.atoms)
        ...
        def _detect_from_grid(prot, grid, within, background_val=0):
            ...
            residue_centroids = [centroid(
                np.array([atm.coordinates for atm in res.atoms])) for res in prot.residues]
            residues = prot.residues
            bs_residues = set()

            for pnt in grid.edge_detection(edge_definition=background_val):
                dists = distance.cdist(
                    np.array([pnt]), np.array(residue_centroids))[0]
                print(dists)
